"""
test-predictions-2B.py -- EgoLongQA Grand Challenge submission inference.

APPROACH: per-option teacher-forced loss scoring with Qwen2-VL-2B-Instruct.
This uses a custom trained JEPA-to-Qwen Bridge. We bypass Qwen2-VL's native
vision encoder entirely by loading the pre-computed JEPA embeddings (.pt files),
passing them through our bridge, and concatenating them directly into the 
VLM's input embeddings space. We then compute the cross-entropy of each 
candidate answer text and predict the one with the lowest loss.
"""
...
PREDICTIONS_PATH = os.path.join(OUTPUT_DIR, "predictions_2B.jsonl")
...
def option_teacher_forced_loss(jepa_tokens_projected, option_text, question, tokenizer, qwen, device):
    """Cross-entropy of ONE candidate option's text as the answer, using JEPA embeddings."""
...
def main():
    ...
    print(f"\nLoading VLM backbone: {QWEN_MODEL_ID}")
        ...
        for i, row in enumerate(manifest, 1):
            ...
            if not os.path.exists(embedding_path):
                raise FileNotFoundError(
                    ...
                    f"Check EMBEDDING_LIBRARY_DIR / MANIFEST_PATH match."
                )
            
            entry = torch.load(embedding_path, map_location=device)
